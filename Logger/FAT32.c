...
void writeFile (unsigned char *fileName, unsigned char *in_data, unsigned char in_length) {
	...
	unsigned int firstClusterLow=0; // value 0 is assigned just to avoid warning in compilation
	struct dir_Structure *dir;
...
//----------------------------------------------------------------------------*/
	...
	while (1) {
		...
		for (sector = 0; sector < sectorPerCluster; sector++) {
			...
			for (i=0; i<bytesPerSector; i+=32) {
				dir = (struct dir_Structure *) &buffer[i];
				if (fileCreatedFlag) {  //to mark last directory entry with 0x00 (empty) mark indicating end of the directory file list
					//dir->name[0] = EMPTY;
					//SD_writeSingleBlock (firstSector + sector);
					return;
				}
